...
## Apply only polling model for Safe states
...
Polling_Predictions <- lapply(clean_poll_2020$state, function(state_name){
  ...
  pred_poll_rep <- ifelse(state_name %in% states$state, pred_poll_rep, (0.85*pred_poll_rep + 0.85* 45.27866))
  pred_poll_dem <- ifelse(state_name %in% states$state, pred_poll_dem, (0.85*pred_poll_dem + 0.85* 54.72134))
...
### Real election Results, read in data
popvote_2020_states <- read_csv("Data/popvote_bystate_1948-2020.csv")

### See how off we are margin wise
...
meansMargins <- offmargins %>% filter(offMargins != is.na(offMargins))

### Root mean squared error
rmse(meansMargins$win_margins2020,meansMargins$win_margin)
### error margin map
plot_usmap(data = offmargins, regions = "state", values = "offMargins") +
  scale_fill_gradient2(
    ...
    breaks = c(-50,-25,0,25,50), 
    limits = c(-50,50),
    name = "Error: Red (Overpredicted Trump) / Blue - (Overpredicted Biden)",
    guide = guide_colourbar(barwidth = 20, barheight = 0.4,
                            title.position = "top"))+
  labs(title = "Margin of Error for State Popular Vote Predictions")+
  theme_void()+
  blogMapGraphics_theme 
ggsave("MarginOfErrorMap.png", height = 8, width = 8)

### Fixed Electoral College Map (NO ERROR NOW!)
plot_usmap(data = Predictions_map, regions = "states", values = "win") +
  ...
  labs(title = "Electoral College Predicted Wins 2020",
       caption = "Joe Biden Electoral College Votes = 279 Donald Trump Electoral College Votes = 259")+
  theme_void()+
  blogMapGraphics_theme+
  theme(legend.position = "none")
ggsave("REALPredictionsWinsElectoralCollege2020.png", height = 8, width = 8)

electoralCollege <- read.csv("Data/ElectoralCollegePost1948.csv")

### Calculate electoral college map results - 279 for biden, 259 for trump
...
ft <- color(ft, i = ~ (win_margin > 0), color = "Blue", j = c(2,3) )
ft <- color(ft, i = ~ (win_margin < 0), color = "Red", j = c(2,3) )
ft

save_as_image(x = ft, path = "Final PredictionsTable.png")

uncertaintyMargins <-offmargins %>%
  mutate(win = ifelse(win_margin > 0, "D", "R")) %>% 
  mutate(mymean = (win_margin+win_margins2020)/2) %>% 
  filter(win_margin != is.na(win_margin))

uncertaintyMargins$state <- factor(uncertaintyMargins$state, 
                                   levels = uncertaintyMargins$state[order(uncertaintyMargins$mymean)])
ggplot(uncertaintyMargins, 
       aes(y=state, x=win_margin), margin(5,5,100,100)) +
  geom_segment(aes(x=win_margin, xend=win_margins2020, y=state, yend=state), color="grey") +
  geom_point(aes(x = win_margin, y = state, color = if_else(win_margin > 0, "blue", "red")),alpha = 0.25, size = 5) +
  geom_point(aes(x = win_margins2020, y = state, color = if_else(win_margins2020 > 0, "blue", "red")), alpha = 1, size = 5) +
  blogGraphics_theme +
  scale_color_manual(values = c("blue","red"), "Party:", c("Dem", "Rep"))+
  xlab("Popular Vote Win Margins")+
  theme(legend.position = "bottom",
        legend.direction = "horizontal",
        legend.text = element_text(size = 12)) +
  ylab("States")+
  labs(title = "Predicted Popular Vote Win Margins Against True Values", caption = "Predicted = Transparent, 2020 Election Result = Filled")


ggsave("PredictedAgainstReal2020.png", height = 12, width = 8)
